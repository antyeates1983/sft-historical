...
if __name__ == '__main__':
    ...
    for irand in range(10, 10+n_rlztns):
        ...
        rng = np.random.default_rng(irand)
 
        os.system('mkdir -p '+outpath.replace(' ', '\ ')+'/regions')
        os.system('mkdir -p '+outpath.replace(' ', '\ ')+'/im-regions')

        # Prepare observational data:
        ncar, cr_start, cr_end, nlow, ica, crs, map_cak = prep_cak_maps(ns, nph, t_start, t_end, bad_ca, max_lat)
        ...
        with open('full-bad-regions.txt', 'r') as f:
            data = f.read()
            bad_list = data.split('\n')
